...
// SQL truyện
$sql_truyen = "SELECT * FROM truyen WHERE TRUYEN_ID='$truyen_id'";
...
$sql_chuong_list = "SELECT CHUONG_ID, CHUONG_STT, CHUONG_LUOTXEM, CHUONG_NGAYDANG, TRUYEN_ID
                    FROM chuong WHERE TRUYEN_ID='$truyen_id' and CHUONG_TRANGTHAI!=0
                    ORDER BY CHUONG_STT DESC";
$result_chuong_list = $conn->query($sql_chuong_list);
while ($row_truyen = $result_truyen->fetch_assoc()) {
    $truyen_name =  $row_truyen['TRUYEN_NAME'];
    $truyen_trangthai = $row_truyen['TRUYEN_TRANGTHAI'];
    $truyen_hinhanh = $row_truyen['TRUYEN_HINHANH'];
    $truyen_mota = $row_truyen['TRUYEN_MOTA'];
}
$truyen_luotxem=0;
...
while ($row_chuong_list = $result_chuong_list->fetch_assoc()) {
    ...
    if(floor($datediff / (60 * 60 * 24))==0){
        $khoangcachngay = "Câp nhật hôm nay";
    }
    else {
        $khoangcachngay = "Cập nhật ".floor($datediff / (60 * 60 * 24)) . " ngày trước";
    }
